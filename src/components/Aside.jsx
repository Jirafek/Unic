        ...
        let items = pages_data.map((el, i) => {
            ...
            return (
                <div onClick={() => {changePage(i)}} key={i} className='flex h-10 items-center cursor-pointer'>
                    ...
                    <p style={ {color: color} } className='text-xs duration-500 font-semibold w-[15rem]'>{el}</p>
                </div>
            );
        });
        ...
        <Fragment>
            <aside className='w-[19.938rem] fixed bg-[#F3F7FF] py-[3.125rem] xl:block hidden'>
                ...
                <div className='h-[16.25rem] px-5 pt-[3.125rem] text-[0.688rem] text-[#3C517B] relative w-60 mt-[3.75rem] mx-auto' style={{ backgroundImage: 'url(/img/faq.png)' }}>
                    ...
                    <button style={{ boxShadow: '0px 20px 20px rgba(0, 0, 0, 0.1)' }}  className='w-full h-10 bg-white rounded-[5px] mt-10 hover:-translate-y-[8px] duration-500'>Задать вопрос</button>
                </div>
            </aside>
        </Fragment>
    );
